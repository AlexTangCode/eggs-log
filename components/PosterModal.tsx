...
import { X, Download, Loader2, Trophy, Calendar, TrendingUp, Star, Award, Crown, CheckCircle2, Egg } from 'lucide-react';
...
const PosterModal: React.FC<PosterModalProps> = ({ isOpen, onClose, hens, logs, onNotify }) => {
  ...
  const handleSaveToGallery = async () => {
    ...
    try {
      ...
      // Increased delay for layout settling and font loading
      await new Promise(r => setTimeout(r, 200));
      
      const canvas = await html2canvas(element, {
        useCORS: true,
        scale: 4, // Higher scale for ultra-crisp assets
        ...
        onclone: (clonedDoc) => {
          ...
          if (clonedEl) {
            ...
            clonedEl.style.transform = 'none'; // Lock transform
      ...
      onNotify('✨ 已成功保存到手机相册！', 'success');
      onClose();
    } catch (err) {
      console.error('Save error:', err);
      onNotify('保存失败，请检查相册访问权限。', 'info');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      {isOpen && (
        ...
        >
          <div className="relative w-full max-w-[340px] flex flex-col items-center">
            ...
            {/* Poster Card - Fixed 375x667 Viewport */}
            <motion.div
              ...
              className="bg-[#F9F5F0] rounded-[40px] overflow-hidden shadow-2xl relative border border-white/20 scale-[0.75] sm:scale-90"
              id="weekly-report-poster"
            >
              {/* Background Accents - Absolute Positioned */}
              ...
              <div className="p-8 h-full relative z-10 flex flex-col">
                ...
                <div className="space-y-6">
                  {/* Main Achievement - Fixed Height Box */}
                ...
                {/* Absolute Positioned Footer to prevent displacement */}
