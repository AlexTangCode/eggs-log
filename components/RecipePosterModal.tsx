...
const RecipePosterModal: React.FC<RecipePosterModalProps> = ({ isOpen, onClose, recipe, onNotify }) => {
  ...
  const handleSaveToGallery = async () => {
    ...
    try {
      ...
      onNotify('✨ 食谱已成功保存到手机相册！', 'success');
      onClose();
    } catch (err) {
      onNotify('保存失败，请检查设置。', 'info');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[500] flex items-center justify-center p-6 bg-[#2D2D2D]/80 backdrop-blur-xl"
        >
          <div className="relative w-full max-w-[340px] flex flex-col items-center">
            ...
            <motion.div
              ...
              className="bg-white rounded-[40px] overflow-hidden shadow-2xl relative border border-white/20 scale-[0.75] sm:scale-90"
              ...
              <div className="p-10 h-full flex flex-col relative z-10">
                <header className="mb-8 flex justify-between items-start h-20">
                  <div className="flex-1 pr-4">
                    ...
                    <h1 className="font-serif text-3xl font-black text-[#2D2D2D] leading-tight m-0 line-clamp-2 text-left">
                      {recipe.recipeName}
                    </h1>
                  </div>
                ...
                <div className="space-y-6 flex-1">
                  ...
                  <div className="relative min-h-[60px]">
                    ...
                    <p className="text-[14px] text-[#2D2D2D] font-medium leading-relaxed italic relative z-10 cn-relaxed pl-5 line-clamp-3 text-left m-0">
                    ...
                    <ul className="space-y-4 m-0 p-0">
                      {displayedSteps.map((step, idx) => (
                        <li key={idx} className="flex gap-3 items-start m-0 p-0">
                          ...
                          <p className="flex-1 text-[13px] text-[#2D2D2D]/90 leading-snug cn-relaxed line-clamp-2 text-left m-0">
                            {step}
                          </p>
                        </li>
                      ))}
                  ...
                  {/* Secret Tip Box */}
                  <div className="bg-[#B66649]/5 h-16 rounded-2xl border border-[#B66649]/10 px-5 flex flex-col justify-center">
